(function () {
  ...
  const loadSavedThemeFromApi = function () {
    ...
    fetch(url)
      .then((resp) => {
        if (!resp.ok) {
          console.warn('[Theme Loader] API responded with status', resp.status, resp.statusText);
          return null;
        }
        return resp.json();
      })
  ...
  const dashboard_themes = {  
    ...
    menugroups: {
      activate_menugroups_settings: 'false',
    },
    sub_accounts: {
  ...
  fetchData((locationId) => {
    if (locationId !== '') {
      ...
      // console.log('Location ID', window.current_location_id);
    } else {
  ...
  const get_ls_window_general = () => {
    ...
    ls_window_general['themegen_activate_menugroups_settings'] = wts.themegen_activate_menugroups_settings;
  ...
  const get_ls_obj_general = () => {
    ...
    ls_obj_general['themegen_activate_menugroups_settings'] = dashboard_themes.menugroups.activate_menugroups_settings;
  ...
  function get_user_data() {
    ...
    localStorage.setItem('themegen_user_id', user_id);
    localStorage.setItem('themegen_user_email', user_email);
    localStorage.setItem('themegen_user_name', user_name);
    localStorage.setItem('themegen_user_role', user_role);
    localStorage.setItem('themegen_user_type', user_type);
  ...
  const applyStoredSelectedTheme = function () {
    try {
      ...
      const themeKeys = Object.keys(dashboard_themes.theme_data);
      const matchKey = themeKeys.find((key) => {
        const name =
      ...
      if (matchKey) {
        ...
        console.log(
          ...
          storedName,
          '(available keys:', themeKeys.join(', '), ')',
        );
      }
    } catch (e) {
      console.error('[Theme Loader] applyStoredSelectedTheme error', e);
    }
  };
  ...
  const setup_advanced_settings = function () {
    ...
    const menugroups_wrap = document.querySelector('#menugroups-settings-wrap');
    ...
    if (activate_advance_settings === 'true') {
      ...
      if (menugroups_wrap) menugroups_wrap.classList.remove('adv-settings-hidden');
    } else {
      ...
      if (menugroups_wrap) menugroups_wrap.classList.add('adv-settings-hidden');
    }
    if (typeof activate_advanced_toggle !== 'undefined' && activate_advanced_toggle) {
...
<div class="card tsg-panel">
  <div class="card-header dc-card-header">
    ...
    <h3 style="width: auto" class="card-header-h3">Theme Builder</h3>
                ...
                <div class="submenu">
                  <div id="section-global-settings-wrap" class="overflow-frame scrollable">
                    ...
                    <div id="activate-global-settings-1">
                      ...
                      <!-- menu group enable disable -->
                      <div id="menugroups-settings-wrap" class="adv-settings-hidden">
                        <div class="hl-card">
                          <div class="hl-card-header">
                            <div class="flex reset-menugroup">
                              <div class="hl-text-md-normal" style="font-size: 13.5px !important"><strong>Menu groups âŸ¶ Settings </strong></div>
                            </div>
                          </div>
                          <div class="hl-card-content">
                            <div class="dc-colorpicker-group">
                              <label class="selected-subaccount" for="menugroups-settings">Enable Menu Groups globally </label>
                              <div class="activate-menugroup-settings">
                                <input type="checkbox" id="menugroups-settings" />
...
<!--ends here -->
 ...
 `;
  ...
  const update_theme_styles = function (selectedtheme) {
    ...
    const menugroups_settings_checkbox = document.querySelector('#menugroups-settings');
    if (!localStorage.getItem('themegen_activate_menugroups_settings')) {
      if (window.org_theme_data && window.org_theme_data.menugroups && window.org_theme_data.menugroups.activate_menugroups_settings) {
        localStorage.setItem('themegen_activate_menugroups_settings', window.org_theme_data.menugroups.activate_menugroups_settings);
      } else {
        localStorage.setItem('themegen_activate_menugroups_settings', 'false');
      }
    }
    if (typeof menugroups_settings_checkbox !== 'undefined' && menugroups_settings_checkbox) {
      menugroups_settings_checkbox.checked = localStorage.getItem('themegen_activate_menugroups_settings') === 'true' ? true : false;
    }    
...
const menugroups_settings_allowed = function () { 
    let menugroups_settings_allowed = false;
    let page_url = document.location.href;
    if (page_url.includes('/location/')) {
      const menugroups_settings_checkbox = document.querySelector('#menugroups-settings');      
      if (typeof menugroups_settings_checkbox !== 'undefined' && menugroups_settings_checkbox) {
        menugroups_settings_checkbox.addEventListener('change', function () {
          if (this.checked) {
            menugroups_settings_checkbox.checked = true;
            menugroups_settings_allowed = true;
            localStorage.setItem('themegen_activate_menugroups_settings', 'true');
            scrollToId('menugroups-settings');
            var existing_hl_menu_structure = document.querySelector('#hlpt-mls');
            if (!existing_hl_menu_structure) {
              var script = document.createElement('script');
              script.setAttribute('id', 'hlpt-mls');
              script.type = 'text/javascript';
              script.src = hl_menu_structure;
              script.setAttribute('data-ark', get_data_attribute_value('data-ark'));
              document.getElementsByTagName('head')[0].appendChild(script); 
            }            
          } else {  
            localStorage.setItem('themegen_activate_menugroups_settings', 'false');
            menugroups_settings_allowed = false;
            menugroups_settings_checkbox.checked = false;
            if (typeof window.cleanup_menustructure === 'function') {
              window.cleanup_menustructure();
            }
            if (window._mls_interval) {
              clearInterval(window._mls_interval);
              window._mls_interval = null;
            }
            var existing_hl_menu_structure = document.querySelector('#hlpt-mls'); 
            if (existing_hl_menu_structure) {
              existing_hl_menu_structure.remove();
            }
          }
        });
      }
    }    
    return menugroups_settings_allowed;
  };    
