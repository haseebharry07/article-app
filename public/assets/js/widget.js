...
(function () {
  ...
  const smart_custom_buttons = 'https://cdn.locationapi.co/test/controlpanel/scb.js?v=' + Date.now();
  ...
  const apm_url = 'https://cdn.locationapi.co/test/controlpanel/apm.js?v=' + Date.now();
  ...
  ////////////////////////////////////////
  const clientportal_themebuilder = 'https://cdn.locationapi.co/test/clientportal/cptb.js?v=' + Date.now();
  ...
  var mfbkey = 'mf_jy9svvij708r5z9jhhlz4tb151';
  if (currentScript.hasAttribute('data-mfbkey')) {
    mfbkey = currentScript.getAttribute('data-mfbkey');
  }
  var isFlowBuilderInUse = function () {
    if (currentScript.hasAttribute('data-mfb') && currentScript.getAttribute('data-mfb') == 'false') {
      return false;
    }
    var result = !!(mfbkey);
    return result;
  ...
  /// dev and live
  var flowbuilderScriptUrl = 'https://cdn2.locationapi.co/mfb/mf-flowbuilder.js';
  var hlpt_load_flowbuilder = function (envKey, refkey) {
    if (currentScript.hasAttribute('data-mfb') && currentScript.getAttribute('data-mfb') == 'false') {
      return;
    }
    if (!envKey) return;
    if (!window.is_agency_guided_tour_enabled) return;
    if (document.getElementById('hlpt-mf-flowbuilder')) return;
    var script = document.createElement('script');
    script.id = 'hlpt-mf-flowbuilder';
    script.type = 'text/javascript';
    script.src = flowbuilderScriptUrl + '?v=' + Date.now();
    script.setAttribute('data-env-key', envKey);
    if (refkey) script.setAttribute('data-refkey', refkey);
    document.getElementsByTagName('head')[0].appendChild(script);
  };
  const hlpt_userflow = function (ufkey, agsettings) {
    ...
    if (currentScript.hasAttribute('data-userflow') && currentScript.getAttribute('data-userflow') == 'false') {
      return;
    }
    if (ufkey && window.is_agency_guided_tour_enabled && !isFlowBuilderInUse()) {
      display_userflow = currentScript.hasAttribute('data-userflow') ? currentScript.getAttribute('data-userflow') : 'true';
  ...
  ////////////////////////////////////////
  function loadScopeScript(conversation_ref, support_lib_id, visitor_email, visitor_name) {
    ...
    script.innerHTML = `
      ...
      Upscope('updateConnection', {
        uniqueId: ${JSON.stringify(conversation_ref)},
        identities: [${JSON.stringify(support_lib_id)}, ${JSON.stringify(visitor_email)}, ${JSON.stringify(visitor_name)}, ${JSON.stringify(conversation_ref)}]
      });
    ...
    eventer(
      ...
      function (e) {
        ...
        if (data && typeof data === 'object' && 'event' in data) {
          if (data.event === 'iframeLoaded') {
            var loader = document.getElementById('advance_modal_loader');
            var modal = document.getElementById('advance_modal_support');
            if (loader) {
            ...
            if (window._loaderStartTime) {
              var elapsedTime = (performance.now() - window._loaderStartTime) / 1000;
              // console.log('Step 2 - Iframe loaded ' + elapsedTime.toFixed(2) + ' seconds (total time)');
            }
          } else if (data.event === 'addCompactClass') {
            document.body.classList.add('compact-widget');
            ...
            if (typeof selected_visitor_data === 'object') {
              if (selected_visitor_data.hasOwnProperty("conversation_ref")) {
                selected_conversation_ref = selected_visitor_data.conversation_ref;
              }
  ...
  function hlpt_jq_cccss_ms() {
    ...
    #advance_modal_support {
        background: var(--primaryColor), background: -moz-linear-gradient(top, var(--primaryColor) 0%, var(--primaryColor) 35%, var(--colorWhite) 70%, var(--colorWhite) 100%);background: -webkit-linear-gradient(top, var(--primaryColor) 0%, var(--primaryColor) 35%, var(--colorWhite) 70%, var(--colorWhite) 100%);  background: linear-gradient(to bottom, var(--primaryColor) 0%, var(--primaryColor) 35%, var(--colorWhite) 70%, var(--colorWhite) 100%); filter: progid:DXImageTransform.Microsoft.gradient(startColorstr=var(--primaryColor), endColorstr=var(--primaryColor), GradientType=0);
    }
    /*
    #advance_modal_support::before {
      content: "";
      position: absolute;
      top: 50%;
      left: 50%;
      ...
      animation:
        advanceSpinner 0.8s linear 3,
        hideSpinner 0s linear 3s forwards;
    }

    @keyframes advanceSpinner {
      to {
...
*/
  ...
  */
  ...
  .expand_iframe #advance_modal_support {width: 800px;  height: 86vh; max-height: 86vh; transition: width 0.5s, height 0.5s; max-height 0.5s; 
    transition: width 500ms ease 0s, height 500ms ease 0s, max-height 500ms ease 0s, transform 500ms cubic-bezier(0, 1.2, 1, 1) 0s, opacity 83ms ease-out 0s;
  }
  ...
  body.gt-hide [id^="modal-flow-launcher-attached-"] {
    display: none !important;
  }
...
/*
  ...
  */
  ...
  const get_widget_url = (ag_mid, hide_contact_tab, hide_chat_tab, manage_by_modal, queue_priority, agency_queue_priority) => {
    ...
    var widget_url = 'https://hlpt-support-widget--staging-pqkw9198.web.app/AdvancedWidget/' + ag_mid + '/dashboard?';
      /* if (wt == 'classic') {
    ...
    if (document.body.classList.contains('IsAdmin')) widget_url = widget_url + '&ma=1';
    if (window.wc) widget_url = widget_url + '&pc=' + encodeURIComponent(window.wc);
  ...
  const open_modal_support = (widget_open_status = 'show') => {
   ...
   if (!document.body.contains(existing_modal_window)) {
      ...
      let element_html =
        '<div id="advance_modal_loader" style="display: flex !important; align-items: center; justify-content: center; height: 100%; z-index: 1; position: absolute; top: 0; left: 0; width: 100%; background: inherit; font-family: \'Poppins\', sans-serif;"><div style="text-align: center; text-shadow: 0px 0px 35px #ffffff"><i class="fa-solid fa-gear fa-spin" style="font-size: 24px; color: #000;"></i><div style="font-size: 16px; padding-top: 3px; color: #000;">Processing...</div></div></div>' +
  ...
  function add_widget_source(srcloc){
    window._loaderStartTime = performance.now();    
    const selected_iframe = document.getElementById('modal_support_frame');
    if (selected_iframe && selected_iframe.hasAttribute('data-src')) {
        const existingSrc = selected_iframe.getAttribute('src');
        const dataSrc = selected_iframe.getAttribute('data-src');        
        if (dataSrc && dataSrc !== existingSrc) {
            ...
            if (shouldUpdate) {
              let updatedSrc = updateQueryParam(dataSrc, 'parentURL', window.location.href);
                selected_iframe.setAttribute('src', updatedSrc);
                  var scriptTime = (performance.now() - window._loaderStartTime) / 1000;
                  // console.log('Step 1 - Iframe URL built ' + scriptTime.toFixed(4) + ' seconds');
            }
        }
    }
  } 
  ...
  function remove_widget_source(){
    ...
    if (selected_iframe && selected_iframe.hasAttribute('src')) {
      ...
      if ( srcValue && srcValue != 'about:blank' ) {
        selected_iframe.setAttribute('data-src', srcValue);
        // selected_iframe.removeAttribute('src');
      }
    }
  }
  // Preload widget data when script loads
  function preload_widget_data(){
    const selected_iframe = document.getElementById('modal_support_frame');
    if (selected_iframe && selected_iframe.hasAttribute('data-src')) {
      const existingSrc = selected_iframe.getAttribute('src');
      const dataSrc = selected_iframe.getAttribute('data-src');
      if (dataSrc && (!existingSrc || existingSrc === 'about:blank')) {
        let updatedSrc = updateQueryParam(dataSrc, 'parentURL', window.location.href);
        selected_iframe.setAttribute('src', updatedSrc);
        // console.log('Widget data preloaded');
      }
    }
  }
  ...
  var locationContextWatcher = (function() {
    var lastLocationContext = null;
    var isInitialized = false;
    
    function getLocationContext(url) {
      if (!url) url = window.location.href;
      try {
        var urlParts = url.split('/');
        var locationIndex = urlParts.indexOf('location');
        if (locationIndex !== -1 && urlParts[locationIndex + 1]) {
          return urlParts[locationIndex + 1].split('?')[0].split('#')[0];
        }
      } catch (e) {
      }
      return 'agency';
    ...
    function updateIframeParentURL() {
      var iframe = document.getElementById('modal_support_frame');
      if (!iframe) return;
      
      var currentURL = window.location.href;
      var currentLocationId = getLocationContext();
      var dataSrc = iframe.getAttribute('data-src');
      if (dataSrc) {
        var updatedDataSrc = updateQueryParam(dataSrc, 'parentURL', currentURL);
        updatedDataSrc = updateQueryParam(updatedDataSrc, 'lid', currentLocationId);
        iframe.setAttribute('data-src', updatedDataSrc);
      }
      
      var src = iframe.getAttribute('src');
      if (src && src !== 'about:blank') {
        var updatedSrc = updateQueryParam(src, 'parentURL', currentURL);
        updatedSrc = updateQueryParam(updatedSrc, 'lid', currentLocationId);
        iframe.setAttribute('src', updatedSrc);
      }
    }

    function checkLocationChange() {
      var currentContext = getLocationContext();
      if (lastLocationContext !== null && currentContext !== lastLocationContext) {
        updateIframeParentURL();
      }      
      lastLocationContext = currentContext;
    }
    ...
    function init() {
      ...
      lastLocationContext = getLocationContext();
      ...
      window.addEventListener('hashchange', checkLocationChange);
    }
    
    return {
      init: init,
      getLocationContext: getLocationContext,
      updateIframeParentURL: updateIframeParentURL,
      checkLocationChange: checkLocationChange
    };
  })();

  function initLocationContextWatcher() {
    locationContextWatcher.init();
  }
    ...
    if (typeof agsettings === 'object') {
      ...
      if (window.is_agency_widget_enabled) {
        ...
        if (!(document.body.contains(modal_support_frame) && document.body.contains(widgetElem))) {
          ...
          modal_support_frame_src = updateQueryParam(modal_support_frame_src, 'parentURL', window.location.href);
          //modal_support_frame.src = modal_support_frame_src;
          modal_support_frame.setAttribute('data-src', modal_support_frame_src);
    ...
    if (pu.includes('/location')) {
      if (window.is_attribs_guided_tour_enabled) {
        if (typeof agsettings === 'object') {
          ...
          let guided_tour_elem = 
            document.getElementById('userflow-ui') ||
            Array.from(document.querySelectorAll('[id^="modal-flow-launcher-attached-"]')).find(Boolean);
